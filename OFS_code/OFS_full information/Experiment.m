...
semilogx(2.^mistakes_idx, mean_mistakes_OFS*100,'b-o'); %X轴取对数
...
semilogx(2.^mistakes_idx, mean_mistakes_Predict_OFS*100,'r-s'); %X轴取对数
...
% mean_mistakes_Predict_OFS = mean(mistakes_list_Predict_OFS,1);
% % semilogx(2.^(mistakes_idx-1), mean_mistakes_OFS,'r-s'); %X轴取对数
% semilogx(2.^mistakes_idx, mean_mistakes_Predict_OFS,'r-o'); %X轴取对数
% % legend('RAND_{sele}','PE_{trun}','OFS','OFS_{predict}');
% legend('OFS_{test}');
% xlabel('样本规模');
% ylabel('在线测试精度')
...
fprintf(1,'训练结果：\n');
fprintf(1,'----------------------------------------------------------------------------------------------------\n');
fprintf(1,'OFS:( number of mistakes \t size of support vectors \t cpu running time_OFS \t accuracy_OFS\t\t)\n');
fprintf(1,'  \t %.4f \t%.4f \t\t %.4f \t %.4f \t\t %.4f \t %.4f\t\t %.4f \t %.4f\n', mean(err_OFS,1), std(err_OFS,1), ...
    mean(nSV_OFS,1), std(nSV_OFS,1), mean(time_OFS,1), std(time_OFS,1),1-((mean(err_OFS,1))/n_xun), ((std(err_OFS))/n_xun));
fprintf(1,'----------------------------------------------------------------------------------------------------\n');
...
fprintf(c,'训练结果：\n');
fprintf(c,'----------------------------------------------------------------------------------------------------\n');
fprintf(c,'OFS:( number of mistakes \t size of support vectors \t cpu running time_OFS \t accuracy_OFS\t\t)\n');
fprintf(c,'  \t %.4f \t%.4f \t\t %.4f \t %.4f \t\t %.4f \t %.4f\t\t %.4f \t %.4f\n', mean(err_OFS,1), std(err_OFS,1), ...
    mean(nSV_OFS,1), std(nSV_OFS,1), mean(time_OFS,1), std(time_OFS,1),1-((mean(err_OFS,1))/n_xun), ((std(err_OFS))/n_xun));
fprintf(c,'----------------------------------------------------------------------------------------------------\n');
fprintf(c,'测试结果：\n');
fprintf(c,'----------------------------------------------------------------------------------------------------\n');
fprintf(c,'Predict_OFS:( number of mistakes \t  cpu running time_OFS \t accuracy_Predict_OFS\t\t)\n');
